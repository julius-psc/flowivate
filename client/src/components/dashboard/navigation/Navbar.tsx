...
const Navbar: React.FC = () => {
  ...
  // REMOVED: statusUpdateError state is no longer needed
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    ...
    },
    {
      id: 3,
      type: "message",
      content: "Sarah mentioned you in a comment",
      time: "3h ago",
      read: true,
    },
  ]);
  ...
  useEffect(() => {
    if (sessionStatus === "authenticated" && userId) {
      ...
      // setStatusUpdateError(null); // No longer needed
      fetch("/api/features/status")
        ...
        .then((data) => {
          ...
          if (!foundStatus)
            console.warn(
              `Workspaceed status "${fetchedStatusName}" not found, defaulting.`
            );
        })
        .catch((error) => {
          console.error("Error fetching initial status:", error);
          // USE TOAST INSTEAD OF setStatusUpdateError
      ...
      setCurrentStatus(statusOptions[0]); // Default to Active if not logged in or loading
    }
  }, [sessionStatus, userId, statusOptions]); // statusOptions added as dependency because it's used inside

  // --- Effect: Handle Clicks Outside Dropdowns ---
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      const target = event.target as HTMLElement;
      // Use optional chaining for safety in case closest returns null
  ...
  const changeStatus = useCallback(
    async (status: StatusOption) => {
      setCurrentStatus(status); // Optimistically set UI state
      setShowStatusMenu(false);
      // setStatusUpdateError(null); // No longer needed

      // Prevent update if not authenticated (shouldn't happen due to UI logic, but safe)
      ...
      try {
        ...
        // Optionally show success toast (if desired)
        ...
        // NOTE: No rollback implemented here; UI remains optimistic. Implement rollback if needed.
      }
    },
    [sessionStatus]
  ); // Added sessionStatus dependency
  ...
  StatusIndicator.displayName = "StatusIndicator"; // For better debugging
    ...
    <>
      <nav className="flex items-center justify-between z-40 px-4 py-2 mx-2 mt-2 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md rounded-xl border border-slate-200/50 dark:border-zinc-800/50">
        ...
        <div className="flex items-center space-x-1">
          ...
          <div className="relative status-container">
            {sessionStatus === "authenticated" && (
              <>
                ...
                {/* REMOVED: Status update error div */}
                {/* {statusUpdateError && ( ... )} */}
          ...
          {/* Notifications Dropdown Area (Unchanged) */}
                      ...
                      {notifications.length > 0 ? (
                        notifications.map((notification) => (
                          <div
                            ...
                            className={`flex items-start p-2 rounded-md ${
                              !notification.read
                                ...
                                : ""
                            } hover:bg-gray-100/60 dark:hover:bg-gray-800/30 cursor-pointer transition-colors duration-150`}
                          ...
                          >
                            ...
                            >
                              {notification.type === "message" ? (
                                <IconMessageCircle className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400" />
                              ) : (
                                <IconBellRinging className="w-3.5 h-3.5 text-green-500 dark:text-green-400" />
                              )}
                            </div>
                            ...
                            {!notification.read && (
                              <span className="w-1.5 h-1.5 rounded-full bg-blue-500 dark:bg-blue-400 ml-2 mt-2 self-start flex-shrink-0"></span>
                            )}
                          </div>
                        ))
                      ) : (
          ...
          {/* Profile Dropdown Area (Unchanged) */}
          <div className="relative profile-container">
            {sessionStatus === "authenticated" && (
              <>
                ...
                >
                  <div className="w-7 h-7 rounded-full bg-green-500 flex items-center justify-center text-white font-medium text-sm">
                    {userInitial}
                  </div>
                </button>
                {showProfile && (
                  <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-900 rounded-lg border border-gray-100 dark:border-gray-800 p-2 z-20">
                    <div className="flex items-center space-x-3 p-2">
                      <div className="w-9 h-9 rounded-full bg-green-500 flex items-center justify-center text-white font-medium">
                        {userInitial}
                      </div>
                      <div>
      ...
      {/* Render Chat Panel if open (Unchanged) */}
      {isChatPanelOpen && (
        <ChatPanel
