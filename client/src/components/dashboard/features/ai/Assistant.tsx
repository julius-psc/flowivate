...
import ChatPanel from './ChatPanel'; // Keep the import
  ...
  return (
    ...
    <div className="bg-white dark:bg-bg-dark border border-gray-200 dark:border-gray-800/50 rounded-xl p-4 relative flex flex-col">
      ...
      <div className="flex-grow overflow-y-auto mb-4">
        {!showRecentChats ? (
          // --- Suggestions View (Structure from OLD, Handlers from NEW) ---
          <div className="mx-4">
            <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-2 w-full mb-4">
              {assistantOptions.map((option, index) => (
                <button
                  key={index}
                  // Use handleNewChat from newer version
          ...
          // --- Recent Chats View (Structure from OLD, Data/Handlers from NEW) ---
          <div className="mx-4 mb-4">
            ...
            {/* Loading/Error states from NEW version */}
            {isLoadingRecents ? (
              ...
              <div className="space-y-2">
                {recentChats.length > 0 ? (
                  recentChats.map(chat => (
                    <div
                      key={chat.id}
                      // Use handleRecentChatClick from newer version
                      onClick={() => handleRecentChatClick(chat.id)}
                      // Classes and structure from OLD version, added group from NEW for delete hover
                    ...
                    >
                      <div className="mr-3 mt-1 flex-shrink-0"> {/* Added flex-shrink-0 from NEW */}
      ...
      {/* Footer Buttons (Using logic from NEW version - always show New Chat) */}
      <div className="flex justify-between items-center flex-shrink-0"> {/* Added flex-shrink-0 */}
        <button
          onClick={() => {
              setShowRecentChats(!showRecentChats);
              // If switching TO recent chats, maybe fetch them immediately?
              if (!showRecentChats) {
                fetchRecentChats();
              }
          }}
        ...
        >
          ...
          <span className="text-sm text-primary-black/60 dark:text-gray-300/60">
             {/* Text toggles correctly based on showRecentChats state */}
            {showRecentChats ? 'New chat' : 'Recent chats'}
          </span>
        </button>

        {/* Always show New Chat button (like NEW version) */}
        <button
            onClick={() => handleNewChat()} // Use the updated handler
      ...
      {/* Chat Panel - Rendered directly inside, passing NEW props */}
      <ChatPanel
        isOpen={isChatOpen}
        setIsOpen={handleChatClose} // Use the closing handler
