...
const transformLayoutForResponse = (
...
export async function GET() {
  try {
    ...
    if (!isValidObjectId(userId)) {
      console.error(
        `Error in GET /api/layout: Invalid session user ID format - ${userId}`
      );
    ...
    const userLayoutDoc = await layoutsCollection.findOne({
      userId: userObjectId,
    });
    ...
    return NextResponse.json(
      { features: userLayoutDoc.features },
      { status: 200 }
    );
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValidObjectId(userId)) {
      console.error(
        `Error in POST /api/layout: Invalid session user ID format - ${userId}`
      );
      return NextResponse.json(
        { message: "Invalid user identifier" },
        { status: 400 }
    ...
    if (!Array.isArray(features) || !features.every((f) => typeof f === "string")) {
      return NextResponse.json(
        { message: "Features must be an array of valid feature keys." },
        { status: 400 }
      );
    }
    ...
    if (!result.acknowledged) {
      console.error(
        `Error in POST /api/layout: DB operation not acknowledged for user ${userId}`
      );
      return NextResponse.json(
        { message: "Failed to save layout due to a database issue." },
        { status: 500 }
      );
    }

    const savedLayout = await layoutsCollection.findOne({
      userId: userObjectId,
    });
    ...
    return NextResponse.json(
      {
        ...
        layout: transformLayoutForResponse(savedLayout),
      },
      { status: 200 }
    );
  } catch (error) {
