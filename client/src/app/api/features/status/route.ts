import { NextResponse, NextRequest } from 'next/server'; // NextRequest for POST
...
export async function GET() {
  try {
    ...
    const user = await usersCollection.findOne({ _id: userObjectId }, { projection: { status: 1 } });
    ...
    return NextResponse.json({ message: 'Failed to retrieve user status' }, { status: 500 });
  }
}

export async function POST(request: NextRequest) { // Use NextRequest for type safety
  try {
    const session = await getServerSession(authOptions);
    ...
    if (!status || typeof status !== 'string' || !allowedStatuses.includes(status)) {
      return NextResponse.json({ message: `Status is required and must be one of: ${allowedStatuses.join(', ')}.` }, { status: 400 });
    }

    const client = await clientPromise;
    const db = client.db(DEFAULT_DB_NAME); // Consistently use defined DB name
