...
function isValidDateString(dateStr: string): boolean {
  ...
  return (
    isValidDateFn(parsedDate) && format(parsedDate, "yyyy-MM-dd") === dateStr
  );
}
  ...
  try {
    ...
    if (!isValidObjectId(sessionUserId)) {
      logApiError(
        ...
        dateString,
        sessionUserId,
        new Error("Invalid user ID format in session")
      );
      return NextResponse.json(
    ...
    const rateLimitResponse = await checkRateLimit(
      sessionUserId,
      `/api/features/journal/${dateString}/GET`,
      20
    );
...
export async function PUT(
  ...
  try {
    ...
    const rateLimitResponse = await checkRateLimit(
      sessionUserId,
      `/api/features/journal/${dateString}/PUT`,
      4
    );
    if (rateLimitResponse) return rateLimitResponse;
    const userObjectId = new ObjectId(sessionUserId);
    ...
    if (!result) {
      logApiError(
        ...
        sessionUserId,
        new Error(
          "Upsert operation failed unexpectedly and returned no document."
        )
      );
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    const rateLimitResponse = await checkRateLimit(
      sessionUserId,
      `/api/features/journal/${dateString}/DELETE`,
      2
    );
    if (rateLimitResponse) return rateLimitResponse;
    const userObjectId = new ObjectId(sessionUserId);
