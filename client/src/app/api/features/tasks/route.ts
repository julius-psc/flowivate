...
export async function POST(request: NextRequest) {
    try {
        ...
        if (typeof name !== 'string' || name.trim() === '' || name.trim().length > MAX_NAME_LENGTH) {
            return NextResponse.json({ message: `Name is required, must be a non-empty string, and not exceed ${MAX_NAME_LENGTH} characters.` }, { status: 400 });
        }
        if (!Array.isArray(tasks)) {
             return NextResponse.json({ message: "Tasks must be an array." }, { status: 400 });
        }

        const processedTasks: TaskItem[] = tasks.map((task: { _id?: string; name: string; completed?: boolean }) => {
            if (!task || typeof task.name !== 'string' || task.name.trim() === '' || task.name.trim().length > MAX_TASK_NAME_LENGTH) {
                throw new Error(`Invalid task structure: Task name is required, must be a non-empty string, and not exceed ${MAX_TASK_NAME_LENGTH} characters.`);
            }
            return {
                _id: new ObjectId(),
                name: task.name.trim(),
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (name !== undefined) {
            if (typeof name !== 'string' || name.trim() === '' || name.trim().length > MAX_NAME_LENGTH) {
                return NextResponse.json({ message: `Name must be a non-empty string and not exceed ${MAX_NAME_LENGTH} characters.` }, { status: 400 });
            }
        ...
        if (tasks !== undefined) {
            ...
            updateFields.tasks = tasks.map((task: { _id?: string; name: string; completed?: boolean }) => {
                if (!task || typeof task.name !== 'string' || task.name.trim() === '' || task.name.trim().length > MAX_TASK_NAME_LENGTH) {
                    throw new Error(`Invalid task structure: Task name is required, must be a non-empty string, and not exceed ${MAX_TASK_NAME_LENGTH} characters.`);
                }
...
export async function DELETE(request: NextRequest) { // Renamed from deleteTaskList to DELETE
    try {
        const userObjectId = await getUserObjectId();
        if (!userObjectId) {
            return NextResponse.json({ message: "Unauthorized or invalid user session" }, { status: 401 });
