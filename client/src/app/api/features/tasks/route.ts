...
const transformTaskListForResponse = (
  doc: TaskListDocument | null
...
export async function GET() {
  try {
    ...
    const taskListsCollection =
      db.collection<TaskListDocument>(COLLECTION_NAME);

    const taskListDocs = await taskListsCollection
      .find({ userId: userObjectId })
      .sort({ createdAt: -1 })
      .toArray();
    const responseTaskLists = taskListDocs
      .map(transformTaskListForResponse)
      .filter((tl) => tl !== null) as TaskListResponse[];

    return NextResponse.json({ taskLists: responseTaskLists }, { status: 200 });
  } catch (error) {
    console.error(`Error in GET /api/features/tasks:`, error);
    return NextResponse.json(
      { message: "Failed to retrieve task lists" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const processedTasks: TaskItem[] = tasks.map(
      (task: {
        ...
        if (
          ...
          task.name.trim() === "" ||
          task.name.trim().length > MAX_TASK_NAME_LENGTH
        ) {
          throw new Error(
        ...
        return {
          ...
          priority:
            typeof task.priority === "number"
              ? Math.max(0, Math.min(3, task.priority))
              : 0,
    ...
    const result = await taskListsCollection.insertOne(
      newTaskListData as TaskListDocument
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!taskListIdString || !isValidObjectId(taskListIdString)) {
      return NextResponse.json(
        { message: "Valid task list ID ('id') is required." },
        { status: 400 }
      );
    }
    ...
    const updateFields: Partial<
      Pick<TaskListDocument, "name" | "tasks" | "updatedAt">
    > = {};
      ...
      updateFields.tasks = tasks.map(
        (task: {
          ...
          const existingTaskId =
            task._id && isValidObjectId(task._id)
    ...
    if (result.matchedCount === 0) {
      return NextResponse.json(
        { message: "Task list not found or access denied" },
        { status: 404 }
      );
    }

    const updatedTaskList = await taskListsCollection.findOne({
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error(
        `Error parsing JSON in DELETE /api/features/tasks:`,
        jsonError
      );
      return NextResponse.json(
        { message: "Invalid JSON payload" },
    ...
    if (!taskListIdString || !isValidObjectId(taskListIdString)) {
      return NextResponse.json(
        { message: "Valid task list ID ('id') is required in the request body." },
        { status: 400 }
      );
    }
