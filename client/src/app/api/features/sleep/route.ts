...
const transformSleepRecordForResponse = (
  recordDoc: SleepDocument | null
): SleepResponse | null => {
...
export async function GET() {
  try {
    ...
    if (!isValidObjectId(userId)) {
      console.error(
        `Error in GET /api/features/sleep: Invalid session user ID format - ${userId}`
      );
    ...
    const responseRecords = sleepDocsArray
      .map(transformSleepRecordForResponse)
      .filter((r) => r !== null) as SleepResponse[];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValidObjectId(userId)) {
      console.error(
        `Error in POST /api/features/sleep: Invalid session user ID format - ${userId}`
      );
      return NextResponse.json(
        { message: "Invalid user identifier" },
        { status: 400 }
    ...
    try {
      ...
      console.error(
        "Error in POST /api/features/sleep: Invalid JSON payload",
        parseError
      );
      return NextResponse.json(
    ...
    if (hours == null || typeof hours !== "number" || hours <= 0 || hours > 24) {
      return NextResponse.json(
        { message: "Hours must be a positive number between 0 (exclusive) and 24." },
        { status: 400 }
      );
    }
    if (!timestamp) {
    ...
    try {
      ...
      console.error(
        "Error in POST /api/features/sleep: Invalid timestamp format",
        { timestampProvided: timestamp, error: dateError }
      );
      return NextResponse.json(
    ...
    if (!result) {
      console.error(
        `Error in POST /api/features/sleep: Database operation failed for user ${userId}. findOneAndUpdate returned null.`
      );
      return NextResponse.json(
        { message: "Database operation failed to save sleep record." },
        { status: 500 }
      );
    }
    ...
    const wasInsert =
      Math.abs(
        upsertedSleepDoc.createdAt.getTime() -
          upsertedSleepDoc.updatedAt.getTime()
      ) < 2000;
